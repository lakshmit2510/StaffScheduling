...
class Booking_model extends CI_Model 
{
  ...
  { 
     $this->db->select('*, booking.BookingRefNo AS BookingRefNo, visatypes.Type AS VisaType, booking.CreatedOn AS BookedOn'); 
     $this->db->from('booking');
     $this->db->join('users','users.UserUID = booking.CreatedBy','LEFT');
    //  $this->db->join('status','status.StatusID = booking.Status','LEFT');
     $this->db->join('visatypes','visatypes.TypeID = booking.ICNumber','LEFT');
     $this->db->join('Airport_Pass_Details','Airport_Pass_Details.PassID = booking.AirportPassNumber','LEFT');
     $this->db->join('company','company.CompanyUID = booking.CompanyUID','LEFT');
     if($filter == 'Today')
     {
       $this->db->where('DATE(booking.StartTime)',date('Y-m-d'));
     } else if($filter == 'Tomorrow') {
       $this->db->where('DATE(booking.StartTime)','CURDATE() + INTERVAL 1 DAY',FALSE);
     } else if($filter == 'Past') {
  ...
  function bookedSlotOnDate($Type,$Date,$DaysCount = 1)
  {
    $DaysCount--;
    $EndDate=date_create($Date);
    date_add($EndDate,date_interval_create_from_date_string("{$DaysCount} days"));
    $EndDate = date_format($EndDate,"Y-m-d");

    $result = $this->db->query("SELECT booking.CheckIn FROM booking LEFT JOIN bookedslots ON booking.BookingID = bookedslots.BookingUID WHERE booking.Status NOT IN (6,3,5) AND booking.Active = 1 AND DATE(bookedslots.CheckIn) BETWEEN '{$Date}' AND '{$EndDate}' AND bookedslots.SlotType =".$Type)->result_array();

    return array_column($result, 'CheckIn');
  }
