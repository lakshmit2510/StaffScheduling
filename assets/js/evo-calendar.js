...
(function (factory) {
  ...
  EvoCalendar = (function () {
    ...
    function EvoCalendar(element, settings) {
        ...
        for (var i = 0; i < _.options.calendarEvents.length; i++) {
          if (_.isValidDate(_.options.calendarEvents[i].date)) {
            _.options.calendarEvents[i].date = _.formatDate(
              new Date(_.options.calendarEvents[i].date),
              _.options.format,
              'en'
            );
          }
      ...
      _.$month =
        isNaN(_.$month) || _.$month == null
          ? _.$cal_current_date.getMonth()
          : _.$month; // 0
      _.$year =
        isNaN(_.$year) || _.$year == null
          ? _.$cal_current_date.getFullYear()
          : _.$year; // 2020
      _.$mainHTML = '';
      _.$sidebarHTML = '';
      _.$calendarHTML = '';
  ...
  EvoCalendar.prototype.buildCalendar = function (val, new_month, new_year) {
    ...
    new_month =
      isNaN(new_month) || new_month == null ? _.$active_month : new_month;
    ...
    function buildMainHTML() {
      var mainHTML =
        '<div class="calendar-sidebar"></div><div class="calendar-inner"></div><div class="calendar-events"></div>';
      if (_.options.canAddEvent) {
        mainHTML +=
          '<span id="eventAddButton" title="Add event">ADD EVENT</span>';
      }
      if (_.options.eventListToggler) {
        mainHTML +=
          '<span id="eventListToggler" title="Close event list"><button class="icon-button"><span class="chevron-arrow-right"></span></button></span>';
      }
      _.$mainHTML = mainHTML;
    }
    ...
    function buildSidebarHTML() {
      ...
      for (var i = 0; i < _.$cal_months_labels.length; i++) {
        ...
        sidebarHTML +=
          '" month-val="' + i + '">' + _.$cal_months_labels[i] + '</li>';
      }
      sidebarHTML += '</ul>';
      if (_.options.sidebarToggler) {
        sidebarHTML +=
          '<span id="sidebarToggler" title="Close sidebar"><button class="icon-button"><span class="bars"></span></button></span>';
      }
    ...
    function buildCalendarHTML() {
      ...
      calendarHTML += _.formatDate(
        new Date(new_year, getMonthNumber(monthName)),
        ...
        for (var j = 0; j <= 6; j++) {
          ...
          if (day <= monthLength && (i > 0 || j >= startingDay)) {
            var thisDay = _.formatDate(
              new Date(new_year, getMonthNumber(monthName), day),
              _.options.format,
              'en'
            );
            calendarHTML += '<div class="day';
            calendarHTML +=
              _.options.disabledDate &&
              _.options.disabledDate.indexOf(thisDay) > -1
                ? ' calendar-disabled'
                : '';
            calendarHTML +=
              (_.$active_date === thisDay ? ' calendar-active' : '') +
              '" date-val="' +
              thisDay +
              '">' +
              day +
        ...
        for (var i = 0; i < _.options.calendarEvents.length; i++) {
          if (_.$active_date === _.options.calendarEvents[i].date) {
            ...
            eventHTML +=
              ...
              _.options.calendarEvents[i].type +
              '"></div></div>';
            eventHTML +=
      ...
      $(
        '.day[date-val="' +
          _.formatDate(
            new Date(
              _.$year,
              getMonthNumber(_.$cal_months_labels[_.$month]),
              _.$cal_current_date.getDate()
            ),
            _.options.format,
            'en'
          ) +
  ...
  EvoCalendar.prototype.initCalendarEvents = function () {
    ...
    if (_.$active_month == 1) {
      // February only!
      if (
        (_.$active_year % 4 == 0 && _.$active_year % 100 != 0) ||
        _.$active_year % 400 == 0
      ) {
        monthLength = 29;
      }
    ...
    for (var i = 0; i < _.options.calendarEvents.length; i++) {
      for (var x = 0; x < monthLength; x++) {
        var active_date = _.formatDate(
          new Date(
            _.$active_year,
            getMonthNumber(_.$cal_months_labels[_.$active_month]),
            x + 1
          ),
          _.options.format,
          'en'
        ...
        if (active_date == _.options.calendarEvents[i].date) {
          ...
          if (
            $('[date-val="' + active_date + '"] span.event-indicator').length ==
            0
          ) {
          ...
          if (
            ...
            var htmlToAppend =
              '<div class="type-bullet"><div class="type-' +
              _.options.calendarEvents[i].type +
              '"></div></div>';
          ...
          if (d == dd) {
            ...
            if (
              $('[date-val="' + active_date + '"] span.event-indicator')
                .length == 0
            ) {
              thisDate.append('<span class="event-indicator"></span>');
            }
  ...
  EvoCalendar.prototype.initEventListener = function () {
    ...
    $('[date-val]:not(.calendar-disabled)')
      .off('click.evocalendar')
